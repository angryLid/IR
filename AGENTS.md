...
3. **行业判定**：判断该公司所属行业是否有对应的研究 skill（如银行业 → `banking-research`）：
   - **有匹配的 skill**：激活该 skill，按其方法论继续后续步骤。
   - **无匹配的 skill**：**不要直接开始研究**。应先分析该行业的商业模式与财务特征，确定适用于该行业的估值方法（DCF、DDM、PB+ROE、RIM、可比公司、可比交易等），并向用户说明所选方法及理由，再继续后续步骤。

后续步骤（数据收集、财务指标、估值、市场定价、报告输出）将随 SOP 逐步补充。
